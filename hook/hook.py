...
def main() -> None:
    ...
    if event == "SessionStart":
        ...
        state["state"] = "idle"
        state["current_task"] = "Session started · awaiting input"

    elif event == "UserPromptSubmit":
        prompt = payload.get("prompt", "")
        state["state"] = "running"
        state["last_prompt"] = truncate(prompt, 100)
        state["current_task"] = truncate(prompt, 80) or "Working"
        ...
        state["current_task"] = (
            truncate(f"{tool_name} · {base}", 80) if base else f"{tool_name} running"
        )
        ...
        if "waiting for your input" in message.lower():
            ...
            if not state.get("current_task"):
                state["current_task"] = "Your turn"
        else:
            state["state"] = "waiting"
            state["current_task"] = truncate(message, 80) or "Awaiting input"
        ...
        if last_msg:
            ...
            state["current_task"] = truncate(f"Done · {state['last_prompt']}", 80)
        else:
